...
- [x] XOAUTH2 framing, bounded newest-50 sequence UID discovery, single UID BODY.PEEK batch, expunge/race/skipped accounting, bounded raw messages, MIME fallback/HTML/plain-text mapping, and reducer stale-event tests pass.
...
- [x] Partial-sync copy hides zero counters and reader copy distinguishes fallback, truncated, and combined content.
...
- [ ] Empty inbox and malformed/truncated mail render bounded fallbacks.
- [ ] Partial-sync banners hide zero counters; fallback/truncated messages show a reader-level indication.
